...
class Loader {
	 ...
	 * @var	array
	 */
	protected $view_paths =	array(VIEWPATH	=> TRUE);
	 ...
	 * @var	array
	 */
	protected $model_paths =	array(APPPATH);
	 ...
	 * @var	array
	 */
	protected $helper_paths =	array(APPPATH, BASEPATH);
	 ...
	 * Is Loaded
	 *
	 * A utility method to test if a class is in the self::$classes array.
	 *
	...
	public function is_loaded($class) {
		return array_search(ucfirst($class), $this->classes, TRUE);
	}
	...
	public function model($model, $name = '', $db_conn = FALSE) {
		...
		$model = ucfirst($model);
		if ( ! class_exists($model, FALSE)) {
			foreach ($this->model_paths as $mod_path) {
		...
		if ( ! is_subclass_of($model, 'Model')) {
			throw new RuntimeException("Class ".$model." doesn't extend Model");
		}
	 ...
	 *					FALSE on failure, Loader instance in any other case
	 */
	...
	public function view($view, $vars = array(), $return = FALSE) {
		return $this->load(array('view' => $view, 'vars' => $this->prepare_view_vars($vars), 'return' => $return));
	}
	 ...
	 * Config Loader
	 *
	 * Loads a config file (an alias for Config::load()).
	 *
	 * @uses	Config::load()
	 ...
	 *				and $object_name is set. Loader instance otherwise.
	 */
	 ...
	 * path arrays.
	 *
	 * @see	Loader::$library_paths
	 * @see	Loader::$model_paths
	 * @see Loader::$helper_paths
	 * @see Config::$_config_paths
	 *
	...
	public function add_package_path($path, $view_cascade = TRUE) {
		...
		array_unshift($this->helper_paths, $path);

		$this->view_paths = array($path.'views/' => $view_cascade) + $this->view_paths;

		// Add config file path
		$config =& $this->get_component('config');
	...
	public function get_package_paths($include_base = FALSE) {
		return ($include_base === TRUE) ? $this->library_paths : $this->model_paths;
	}
	...
	public function remove_package_path($path = '') {
		...
		if ($path === '') {
			array_shift($this->library_paths);
			array_shift($this->model_paths);
			array_shift($this->helper_paths);
			array_shift($this->view_paths);
			array_pop($config->_config_paths);
		} else {
			$path = rtrim($path, '/').'/';
			foreach (array('library_paths', 'model_paths', 'helper_paths') as $var) {
				if (($key = array_search($path, $this->{$var})) !== FALSE) {
					unset($this->{$var}[$key]);
				}
			}

			if (isset($this->view_paths[$path.'views/'])) {
				unset($this->view_paths[$path.'views/']);
			}
		...
		$this->library_paths = array_unique(array_merge($this->library_paths, array(APPPATH, BASEPATH)));
		$this->helper_paths = array_unique(array_merge($this->helper_paths, array(APPPATH, BASEPATH)));
		$this->model_paths = array_unique(array_merge($this->model_paths, array(APPPATH)));
		$this->view_paths = array_merge($this->view_paths, array(APPPATH.'views/' => TRUE));
	 ...
	 *
	 * Variables are prefixed with  to avoid symbol collision with
	...
	protected function load($data) {
		...
		log_message('info', 'File loaded: '.$path);
	 ...
	 *
	 * @used-by	Loader::load_stock_library()
	 * @used-by	Loader::load_library()
	 *
	 ...
	 * variable names (i.e. keys prefixed with '').
	 *
	 * @param	mixed	$vars
	 * @return	array
	 */
